...
| 备案 / 商户号 | 运维 + 主体资质 | 同主体域名 `szbolent.cn` |

**身份真源（纠正常见误解）：**

```text
小程序 wx.login → code
    → HTTPS api.szbolent.cn/v1/auth/wechat   ← 生产主路径
...
| 域名 | 用途 | 部署目标 | 负责仓 |
|------|------|----------|--------|
| `szbolent.cn` | 门户首页（www 或 apex） | 静态 `dist/`（COS+CDN 或 Nginx） | szbolent-portal |
| `www.szbolent.cn` | 同上（CNAME → CDN） | 同上 | szbolent-portal |
| `api.szbolent.cn` | **Looma REST API** | 腾讯云 CVM/Lighthouse + Nginx → Flask | looma-zervi |
| `cms.szbolent.cn` | WordPress headless（可选 P1+） | 轻量/docker WP | 运维 |
| `m.szbolent.cn` | 小程序业务 H5（可选） | 静态或反代 | 按需 |
...
| 类型 | 域名 |
|------|------|
| request 合法域名 | `https://api.szbolent.cn` |
| uploadFile（若需） | `https://api.szbolent.cn` |
| downloadFile（若需） | `https://api.szbolent.cn` 或 CDN 域 |

**配置完成后：** 小程序 **直连** `api.szbolent.cn`，CloudBase 登录透传可下线。
...
```nginx
# /etc/nginx/sites-enabled/api.szbolent.cn
server {
    listen 443 ssl http2;
    server_name api.szbolent.cn;
...
```env
VITE_SITE_URL=https://szbolent.cn
VITE_LOOMA_API_BASE=https://api.szbolent.cn
VITE_BLOG_API_BASE=https://cms.szbolent.cn/wp-json/wp/v2
```
...
- [ ] 腾讯云账号已完成实名（企业）
- [ ] 域名 `szbolent.cn` 已在腾讯云注册/转入
- [ ] 云服务器（CVM/Lighthouse）已购，大陆节点
- [ ] 营业执照、法人身份证、网站负责人身份证
- [ ] 网站名称、简介（建议：诗词文化 / 个人品牌门户，与小程序类目一致）
- [ ] 隐私政策、用户协议 URL（可先部署静态页到 `szbolent.cn/legal/*`）
...
| API 域 | `api.szbolent.cn` 作为二级域名一并备案 |
| 服务器 IP | 填入腾讯云实例公网 IP |
...
- [ ] 更新 `frontend/packages/miniprogram/utils/config.ts` → `API_BASE = 'https://api.szbolent.cn'`
- [ ] 微信公众平台配置合法域名
- [ ] 运行联调脚本（见 §6）
...
| 环境 | 登录路径 |
|------|----------|
| **生产** | 小程序 → `https://api.szbolent.cn/v1/auth/wechat` |
...
WECHAT_PAY_NOTIFY_URL=https://api.szbolent.cn/v1/payment/notify/wechat
...
- [ ] **P0-2** 提交 `szbolent.cn`、`api.szbolent.cn` ICP 备案
- [ ] **P0-3** 备案通过：DNS + SSL 证书部署
- [ ] **P0-4** 申请微信小程序（或与现有 PlanetX 小程序协调类目/主体）
- [ ] **P0-5** 微信公众平台配置 `api.szbolent.cn` 合法域名

#### looma-zervi（后端）

- [ ] **P0-6** 服务器部署 Flask（`PORT=5200`，systemd/docker）
- [ ] **P0-7** Nginx 反代 `api.szbolent.cn` → `:5200`（见 §1.3）
- [ ] **P0-8** 生产 `.env`：`WECHAT_DEV_MODE=false`，填入真实 `WECHAT_APPID/SECRET`
- [ ] **P0-9** `GET https://api.szbolent.cn/health` 返回 200
- [ ] **P0-10** `POST /v1/auth/wechat` 真机小程序登录成功，返回 JWT
- [ ] **P0-11** 小程序 `config.ts` 指向 `https://api.szbolent.cn`（弃用 CloudBase 透传）
- [ ] **P0-12** 运行 `scripts/verify-closed-loop.sh`（`API_BASE=https://api.szbolent.cn`）

#### szbolent-portal（门户）

- [ ] **P0-13** 生产构建：`VITE_LOOMA_API_BASE=https://api.szbolent.cn`
- [ ] **P0-14** 部署 `dist/` 至 `szbolent.cn`（CDN 或 Nginx）
...
- [ ] **P1-4** 回调 URL 白名单：`https://api.szbolent.cn/v1/payment/notify/wechat`
...
- [ ] **P2-7** 博客 headless（`cms.szbolent.cn`）+ 会员文章标记
...
```env
VITE_LOOMA_API_BASE=
# 留空则走同源 /v1 proxy；生产填 https://api.szbolent.cn
```
