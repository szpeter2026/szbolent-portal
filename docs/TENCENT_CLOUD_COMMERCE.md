...
| 备案 / 商户号 | 运维 + 主体资质 | API 已部署在 `api.genz.ltd`；门户内容站 `szbolent.cn`（个人备案）；企业主域 `szbolent.com.cn`（规划中） |

**身份真源（纠正常见误解）：**

```text
小程序 wx.login → code
    → HTTPS api.genz.ltd/v1/auth/wechat   ← 生产主路径
...
| 域名 | 用途 | 部署目标 | 备案主体 | 负责仓 |
|------|------|----------|---------|--------|
| `szbolent.cn` | 根域（301 → www） | 跳转至 `www.szbolent.cn` | **个人** | szbolent-portal |
| `www.szbolent.cn` | **门户首页（canonical）** | WordPress / Vue3 静态（Nginx） | **个人** | szbolent-portal |
| `szbolent.com.cn` | 企业主域（301 → api/www） | 跳转或保留 | **公司** | 运维 |
| `www.szbolent.com.cn` | 企业门户（可选，P1+） | 静态或反代 | **公司** | 按需 |
| `api.genz.ltd` | **Looma REST API** | 腾讯云 CVM/Lighthouse + Nginx → Flask | **公司** | looma-zervi |
| `cms.szbolent.com.cn` | WordPress headless（可选 P1+） | 轻量/docker WP | **公司** | 运维 |
| `m.szbolent.com.cn` | 小程序业务 H5（可选） | 静态或反代 | **公司** | 按需 |
...
| 类型 | 域名 |
|------|------|
| request 合法域名 | `https://api.genz.ltd` |
| uploadFile（若需） | `https://api.genz.ltd` |
| downloadFile（若需） | `https://api.genz.ltd` 或 CDN 域 |

**配置完成后：** 小程序 **直连** `api.genz.ltd`，CloudBase 登录透传可下线。
...
```nginx
# /etc/nginx/sites-enabled/api.genz.ltd
server {
...
- [ ] 隐私政策、用户协议 URL（可先部署静态页到 `www.szbolent.cn/legal/*`）

### 2.2 备案材料对照

| 项 | 内容 |
|----|------|
| 门户网站（内容站） | `https://www.szbolent.cn`（**个人备案**） |
| 企业主域 | `https://www.szbolent.com.cn`（**企业备案**，P1+） |
| API 域 | `api.genz.ltd` 作为企业备案二级域名 |
| 首页 URL（备案提交用） | `https://www.szbolent.com.cn` 或 `https://api.genz.ltd` |
...
- [ ] 更新 `frontend/packages/miniprogram/utils/config.ts` → `API_BASE = 'https://api.genz.ltd'`
- [ ] 微信公众平台配置合法域名
- [ ] 运行联调脚本（见 §6）
...
| 环境 | 登录路径 |
|------|----------|
| **生产** | 小程序 → `https://api.genz.ltd/v1/auth/wechat` |
...
WECHAT_PAY_NOTIFY_URL=https://api.genz.ltd/v1/payment/notify/wechat
...
- [ ] **P0-2** 提交 `szbolent.cn`（个人）、`szbolent.com.cn` + `api.genz.ltd`（企业）ICP 备案
- [ ] **P0-3** 备案通过：DNS + SSL 证书部署
- [ ] **P0-4** 申请微信小程序（或与现有 PlanetX 小程序协调类目/主体）
- [ ] **P0-5** 微信公众平台配置 `api.genz.ltd` 合法域名

#### looma-zervi（后端）

- [ ] **P0-6** 服务器部署 Flask（`PORT=5200`，systemd/docker）
- [ ] **P0-7** Nginx 反代 `api.genz.ltd` → `:5200`（见 §1.3）
- [ ] **P0-8** 生产 `.env`：`WECHAT_DEV_MODE=false`，填入真实 `WECHAT_APPID/SECRET`
- [ ] **P0-9** `GET https://api.genz.ltd/health` 返回 200
- [ ] **P0-10** `POST /v1/auth/wechat` 真机小程序登录成功，返回 JWT
- [ ] **P0-11** 小程序 `config.ts` 指向 `https://api.genz.ltd`（弃用 CloudBase 透传）
- [ ] **P0-12** 运行 `scripts/verify-closed-loop.sh`（`API_BASE=https://api.genz.ltd`）

#### szbolent-portal（门户）

- [ ] **P0-13** 生产构建：`VITE_LOOMA_API_BASE=https://api.genz.ltd`
- [ ] **P0-14** 部署 `dist/` 至 `www.szbolent.cn`（Nginx）或 WordPress 主题渲染
...
- [ ] **P0-17** 隐私政策 / 用户协议页面上线（备案与小程序审核需要；部署至 `www.szbolent.cn/legal/*`）
...
# 2. 诗词公开浏览
curl -sf "https://api.genz.ltd/v1/poetry/random?count=1"

# 3. 门户可访问
curl -sfI https://www.szbolent.cn | head -1
```
...
- [ ] **P1-4** 回调 URL 白名单：`https://api.genz.ltd/v1/payment/notify/wechat`
...
- [ ] **P1-14** 新增 `/pricing` 套餐页（调 looma plans + orders；注意：个人备案 `www.szbolent.cn` **不放置**定价/支付，此页走小程序或企业域）
...
- [ ] **P2-7** 博客 headless（`www.szbolent.cn` WordPress 同域或 `cms.szbolent.com.cn`）+ 会员文章标记
...
```env
VITE_LOOMA_API_BASE=
# 留空则走同源 /v1 proxy；生产填 https://api.genz.ltd（门户 www.szbolent.cn 跨域调用）
```
