...
- **P0 重点：** CORS、备案、`api.szbolent.cn`、portal 对接 Looma auth/payment、清理 legacy `:8001`  
...
| 小程序仍独立维护 JWT/consent | 直连 `api.szbolent.cn` 后与 Web 端 tier/consent 行为不一致 |
...
| 报告 # | 线 B 关联 |
|--------|-----------|
| 1 小程序 shared-core | 小程序直连 api.szbolent.cn 前必完成 |
