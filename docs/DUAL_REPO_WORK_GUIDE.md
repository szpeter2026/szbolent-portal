...
本指引供 **looma-zervi 团队**（平台 / API / 小程序 / PlanetX·T-space）与 **szbolent-portal 团队**（`www.szbolent.cn` 门户壳）在后续迭代中对照执行，确保：
...
| 变量 | 开发 | 生产 |
|------|------|------|
| `VITE_LOOMA_API_BASE` | 留空（走 vite proxy `/v1` → `:5200`） | `https://api.genz.ltd` |
| `VITE_SITE_URL` | `http://localhost:3000` | `https://www.szbolent.cn` |
| `VITE_BLOG_API_BASE` | `/wp-json/wp/v2` | `https://www.szbolent.cn/wp-json/wp/v2`（WP 同域部署）或 `https://cms.szbolent.com.cn/wp-json/wp/v2`（独立 CMS） |
...
```env
CORS_ORIGINS=https://szbolent.cn,https://www.szbolent.cn,https://szbolent.com.cn,https://www.szbolent.com.cn,http://localhost:3000,http://localhost:5173,http://localhost:5174
```

**验收：** 浏览器从 `https://www.szbolent.cn` 带 `Authorization` 调 `https://api.genz.ltd/v1/auth/me` 无 CORS 错误。
...
- [ ] **P0-L1** 生产 `CORS_ORIGINS` 含 `szbolent.cn`、`www.szbolent.cn`、`szbolent.com.cn`、`www.szbolent.com.cn` 与 `localhost:3000`（§5.3）
- [ ] **P0-L2** Nginx 反代 `api.genz.ltd` → `:5200`（见 `TENCENT_CLOUD_COMMERCE.md` §1.3）
- [ ] **P0-L3** `GET https://api.genz.ltd/health` 返回 200
...
- [ ] **P0-P1** 生产构建注入 `VITE_LOOMA_API_BASE=https://api.genz.ltd`
...
# 闭环（looma 仓）
API_BASE=https://api.genz.ltd ./scripts/verify-closed-loop.sh
```
...
- [ ] **P2-4** CloudBase 登录透传正式下线（小程序直连 `api.genz.ltd`）
